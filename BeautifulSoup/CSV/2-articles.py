...
links_goods = []
with open('BeautifulSoup/CSV/res2.csv', 'w', encoding='utf-8-sig', newline='') as file:
...
for links in links_goods:
    for link in links:
        ...
        with open('BeautifulSoup/CSV/res2.csv', 'a', encoding='utf-8-sig', newline='') as file:
            writer = csv.writer(file, delimiter=';')
            for item, art, descr, am, new_price, old_price in zip(name, article, description, amount, new_price, old_price):
                flatten = item, art, *[x.split(':')[1].strip() for x in descr if x], am, new_price, old_price, url
