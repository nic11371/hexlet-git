...
with webdriver.Chrome() as browser:
    ...
    max_skills = 0
    best_cookie_value = None

    for cookie in cookies:
        # Очищаем и добавляем по одному куки
        browser.delete_all_cookies()
        browser.add_cookie(cookie)
        browser.refresh()

        # Получаем данные хакера
        try:
            age = int(browser.find_element(By.ID, 'age').text[5:])
            skills = len(browser.find_element(
                By.ID, 'skillsList').find_elements(By.TAG_NAME, 'li'))

            # Проверяем, является ли текущий хакер лучшим
            if (age < youngest_age) or (age == youngest_age and skills > max_skills):
                youngest_age = age
                max_skills = skills
                best_cookie_value = cookie['value']

        except Exception as e:
            print(f"Ошибка при обработке куки {cookie['name']}: {e}")
            continue

    print(f"Лучший хакер: возраст {youngest_age}, языков {max_skills}")
    print(f"Значение куки: {best_cookie_value}")
    time.sleep(3)
