...
async def board_bus(passenger):
    await asyncio.sleep(passenger['Speed'])
    print(f"{passenger['Name']} сел в автобус.")
    bus_list.append(f"{passenger['Name']} сел в автобус.")
    return passenger['Name']

bus_list = []
async def main():
    tasks = [asyncio.create_task(board_bus(passenger)) for passenger in passengers]
    try:
        boarded_passengers = await asyncio.wait_for(asyncio.gather(*tasks), timeout=5)
    except asyncio.TimeoutError:
        boarded_passengers = [task.result() for task in tasks if not task.cancelled()]

    for passenger in passengers:
        if passenger['Name'] not in boarded_passengers:
            print(f"{passenger['Name']} {passenger['Job']} не успел/а вовремя сесть в автобус.")
            bus_list.append(f"{passenger['Name']} {passenger['Job']} не успел/а вовремя сесть в автобус.")
